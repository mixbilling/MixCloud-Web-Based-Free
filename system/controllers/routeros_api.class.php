...
class RouterosAPI
{
     ...
     */
    public function connect($ip, $port_mik, $login, $password)
    {
        for ($ATTEMPT = 1; $ATTEMPT <= $this->attempts; $ATTEMPT++) {
            $this->connected = false;
            $this->debug('Connection attempt #' . $ATTEMPT . ' to ' . $ip . ':' . $port_mik . '...');
            $this->socket = @fsockopen($ip, $port_mik, $this->error_no, $this->error_str, $this->timeout);
            if ($this->socket) {
