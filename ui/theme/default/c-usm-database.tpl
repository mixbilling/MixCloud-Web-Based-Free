						...
						<div class="col-sm-12">
							<div class="panel">
								...
								<div style="min-height:320px" class="panel-body">								
									<form  action="{$_url}user-manager/c-restore-db" method="post" name="postform" enctype="multipart/form-data" >					
										<div class="form-group">								
											<div>
												...
												<a title="BACKUP USERMANAGER DATABASE" href="{$_url}user-manager/c-db-backup" style="border-radius:0;width:125px;height:30px;margin-top:8px;padding-top:3px;" class="btn btn-success btn-xs"><i class="fa fa-download"></i> Backup DB</a>											
											</div>
										</div>									
									</form>									
									<hr>
										...
										<table  class="display table table-bordered table-bordered" id="dynamic-table">
											...
											{foreach $usm_db as $db}
												<tr>
													...
													<td>
														<a href="{$_url}user-manager/c-restore-backup&id={$db['.id']}&name={$db['name']}" id="{$db['.id']}" title="RESTORE THIS DATABASE" style="color:#fff;font-size:17px;width:30px;height:30px;" class="btn btn-success btn-sm fa fa-upload" aria-hidden="true"></a>
														
														<a href="{$_url}user-manager/c-remove-backup&id={$db['.id']}&name={$db['name']}" id="{$db['.id']}" title="REMOVE THIS BACKUP : {$db['name']} ?" onClick="return confirm('{$_L['API_Remove_Alert']}')" style="color:#fff;font-size:17px;width:30px;height:30px;" class="btn btn-warning btn-sm cdelete fa fa-trash" aria-hidden="true"></a>	
													</td>
												</tr>
											{/foreach}
											</tbody>
									...
									<div class="adv-table table-responsive">
										<h5>Database Info</h5>
										<table  class="display table table-bordered table-bordered" id="static-table">
											<thead>
												<tr style="font-size:13px;">
