			...
			<script>
				function remove() {
					if(confirm('{$_L["API_Remove_Alert"]}')){
						document.getElementById("usm_profile_limit").setAttribute("action", "{$_url}user-manager/remove-selected-profile-limit");
											...
											<table  class="display table table-bordered table-bordered" id="asc-sort-col-1">
												...
												{foreach $profiles_limit as $profile}
													...
													{/if}	
														...
														<td>
															<a href="{$_url}user-manager/c-profiles-limit-edit&id={$profile['.id']}" id="{$profile['.id']}" title="EDIT THIS PROFILE LIMITATION" style="color:#fff;font-size:17px;width:30px;height:30px;" class="btn btn-success btn-sm fa fa-pencil-square-o" aria-hidden="true"></a>
														{if {$c_permissions} eq 'owner' || {$c_permissions} eq 'full'}	
															<a href="{$_url}user-manager/c-profiles-limit-remove&id={$profile['.id']}" id="{$profile['.id']}" title="REMOVE THIS PROFILE LIMITATION" onClick="return confirm('{$_L['API_Remove_Alert']}')" style="color:#fff;font-size:17px;width:30px;height:30px;" class="btn btn-warning btn-sm cdelete fa fa-trash" aria-hidden="true"></a>
														{/if}	
														</td>
													</tr>
												{/foreach}
