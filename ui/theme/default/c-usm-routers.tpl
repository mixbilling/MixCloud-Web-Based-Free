{include file="sections/header.tpl"}
			...
			<script>				
				function enable() {
					document.getElementById("routers").setAttribute("action", "{$_url}user-manager/enable-selected-routers");
					document.forms['routers'].submit();
				}
				function disable() {
					document.getElementById("routers").setAttribute("action", "{$_url}user-manager/disable-selected-routers");
					document.forms['routers'].submit();
				}
				function remove() {
					if(confirm('{$_L["API_Remove_Alert"]}')){
						document.getElementById("routers").setAttribute("action", "{$_url}user-manager/remove-selected-routers");
						document.forms['routers'].submit();
					}	
				}					
			</script>
											...
											<table  class="display table table-bordered table-bordered" id="asc-sort-col-1">
												...
												{foreach $usm_routers as $routers}
													<tr>
														...
														<td>
														<a href="{$_url}user-manager/c-routers-enable&id={$routers['.id']}&name={$routers['name']}" id="{$routers['.id']}" title="Enable This Router ?"><span style="width:65px;" class="btn btn-danger btn-sm cdelete">Disable</span></a>
														</td>													
														
														{else}
														
														<td>													
														<a href="{$_url}user-manager/c-routers-disable&id={$routers['.id']}&name={$routers['name']}" id="{$routers['.id']}" title="Disable This Router ?"><span style="width:65px;" class="btn btn-success btn-sm cdelete">Enable</span></a>
														</td>
														
														{/if}												
														<td>
															<a href="{$_url}user-manager/c-routers-edit&id={$routers['.id']}&name={$routers['name']}" id="{$routers['.id']}" title="EDIT THIS ROUTER" style="color:#fff;font-size:17px;width:30px;height:30px;" class="btn btn-success btn-sm fa fa-pencil-square-o" aria-hidden="true"></a>
														{if $routers['permissions'] neq 'owner'}
															<a href="{$_url}user-manager/c-routers-remove&id={$routers['.id']}&name={$routers['name']}" id="{$routers['.id']}" title="REMOVE THIS ROUTER" onClick="return confirm('{$_L['API_Remove_Alert']}')" style="color:#fff;font-size:17px;width:30px;height:30px;" class="btn btn-warning btn-sm cdelete fa fa-trash" aria-hidden="true"></a>
														{/if}	
														</td>
													</tr>
												{/foreach}
				...
				<div  class="modal-dialog" role="document">
					<div class="modal-content">
						...
						<div class="modal-body">
							<form id="modal-form" class="form-horizontal" method="post" role="form" action="{$_url}user-manager/c-routers-add" >
