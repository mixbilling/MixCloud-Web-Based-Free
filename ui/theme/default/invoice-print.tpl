				...
				<div class="invoice-box">
					<table cellpadding="0" cellspacing="0">
						...
						<tr style="background:#ffffff;" class="information">
							<td colspan="2">
								<table class="table display table-stripped" style="margin-top:5px;max-width:60%">
									...
									<tr>
										<td style="width:25%">{$d['fullname']}</td>
										<td style="width:25%;text-align:left">{$d['type']}</td>
										<td style="width:25%">{if $d['expiration'] eq '0000-00-00'}-{else}{$payment}{/if}</td>
										<td style="width:25%">{if $d['expiration'] eq '0000-00-00'}-{else}{date($_c['date_format'], strtotime($due_payment))}{/if}</td>									
									</tr>
								</table>
							</td>
							<td style="max-width:100%">
								<h4 style="position:absolute;text-align:right;font-weight:normal;border-top:solid 1px #dddddd;padding-top:0px;padding-bottom:5px;font-size:40px;right:45px;top:93px">#{$d['invoice']}</h4>
						...
						<table class="table display table-stripped" style="margin-top:-10px;width:100%">
							...
							<tr>
								<td>{$d['plan_name']}</td>
								<td>{$_c['currency_code']} {number_format($d['price'],2,$_c['dec_point'],$_c['thousands_sep'])}</td>
							</tr>
							<tr>
								<td>
									{if {$d['type']} eq 'PPPOE'}
								...
								<td>
									{$_c['currency_code']} {number_format($d['ppn'],2,$_c['dec_point'],$_c['thousands_sep'])}
								</td>								
							</tr>
							<tr style="border-bottom:solid 1px #dddddd">
								<td></td>
								<td style="font-size:25px">Total : {$_c['currency_code']} {number_format($d['total'],2,$_c['dec_point'],$_c['thousands_sep'])}</td>
							</tr>							
						</table>					
