			...
			<script>		
				function remove() {
					if(confirm('{$_L["Remove_Alert"]}')){
						document.getElementById("member").setAttribute("action", "{$_url}prepaid/remove-selected");
			...
			<div class="content-wrapper">
				<section class="content">
					<div class="row">
						...
						<div class="col-sm-12">
							<div class="panel">
								<div class="panel-heading">{$_L['Prepaid_Member']} PPPOE</div>
								<div class="panel-body">
									<div class="group">
										<div style="margin-top:4px;margin-bottom:-20px" class="dropdown "><a style="width:170px" href="#" class="dropdown-toggle btn btn-success waves-effect" data-toggle="dropdown" aria-expanded="false"><i class="ion ion-navicon-round"></i> {$_L['Option_Menu']}<span class="caret"></a></span>
											<ul style="margin-left:2px;width:auto;min-width:168px" class="dropdown-menu">
												<li><a href="#add-member" data-toggle="modal" data-target="#add-member" title="ADD NEW PPPOE MEMBER"><i class="ion ion-android-add"> </i> {$_L['Add_Member']}</a></li>
												...
												<li><a href="{$_url}prepaid/recharge-pppoe" title="RECHARGE PPPOE MEMBER"><i class="ion ion-battery-low"></i> {$_L['Recharge_Member']}</a></li>
												<li><a href="#" onclick="remove();" title="REMOVE SELECTED PREPAID PPPOE"><i class="ion ion-trash-a"></i> {$_L['Remove_Selected']}</a></li>												
											</ul>
										</div>						
									</div>
									<br>
									...
									<div class="adv-table table-responsive">
										<form id="member" method="post" role="form" action="{$_url}prepaid/remove-selected" >
											<table  class="display table table-bordered table-bordered" id="prepaid-pppoe">
												<thead>
													<tr>
														...
														<th>{$_L['Plan_Name']}</th>
														<th>{$_L['Type']}</th>
														<th>{$_L['Recharged_On']}</th>
														<th>{$_L['Expires_On']}</th>
														<th>{$_L['Recharge_By']}</th>
														<th>{$_L['Routers']}</th>										
														<th>{$_L['Recharge']}</th>										
														<th>{$_L['Manage']}</th>
													</tr>
								...
								<form id="modal-form" class="form-horizontal" method="post" role="form" action="{$_url}customers/add-post" > 
									...
									<div class="tab-content">
										<div id="service-plan" class="tab-pane fade in active">
												...
												<div class="form-group">
													...
													{if $_admin['user_type'] eq 'Admin'}
														<select id="owner" name="owner" class="form-control" required>							
															<option value=''>- {$_L['Select_Owner']} -</option>
														{foreach $o as $os}
															<option value="{$os['id']}">{$os['username']}</option>
														{/foreach}						
														</select>
													{else}
														<select id="owner" name="owner" class="form-control" readonly>
											...
											<div class="form-group" style="display:none" id="ip_address_type">
										...
										<div id="account" class="tab-pane fade">
											...
											<div class="col-md-3 col-sm-6">	
												<div class="form-group">
													...
													<div class="col-md-6">
														<input type="text" class="form-control" id="phonenumber" name="phonenumber" placeholder="0813-6446-0755">
													</div>
												</div>
											</div>
											<div class="col-md-3 col-sm-6">													
												<div class="form-group">
													...
													<div class="col-md-6">
														<input type="text" class="form-control" id="address" name="address" placeholder="Gurun Aua">
													</div>
												</div>
											</div>
									...
									<div  style="margin-bottom:-14px" class="form-group"></div>	
									<hr>
										<div>
