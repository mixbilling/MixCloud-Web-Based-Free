{include file="sections/header.tpl"}
			...
			<script>				
				function enable() {
					document.getElementById("customers").setAttribute("action", "{$_url}mikrotik/enable-selected-customers");
					document.forms['customers'].submit();
				}
				function disable() {
					document.getElementById("customers").setAttribute("action", "{$_url}mikrotik/disable-selected-customers");
					document.forms['customers'].submit();
				}
				function remove() {
					if(confirm('{$_L["API_Remove_Alert"]}')){
						document.getElementById("customers").setAttribute("action", "{$_url}mikrotik/remove-selected-customers");
						document.forms['customers'].submit();
					}	
				}					
			</script>
												...
												{foreach $usm_customers as $customers}
													...
													{if {$customers['disabled']} eq 'true' }													
														...
														{if $customers['permissions'] neq 'owner'}
															<a href="{$_url}mikrotik/usm-customers-enable&id={$customers['.id']}&name={$customers['login']}" id="{$customers['.id']}" title="ENABLE THIS CUSTOMER"><span style="width:65px;" class="btn btn-danger btn-sm cdelete">Disable</span></a>
														{/if}
														</td>													
													{else}
														<td>
														{if $customers['permissions'] neq 'owner'}
															<a href="{$_url}mikrotik/usm-customers-disable&id={$customers['.id']}&name={$customers['login']}" id="{$customers['.id']}" title="DISABLE THIS CUSTOMER"><span style="width:65px;" class="btn btn-success btn-sm cdelete">Enable</span></a>
														{/if}
														</td>
													{/if}												
														<td>
															<a href="{$_url}mikrotik/usm-customers-edit&id={$customers['.id']}&name={$customers['login']}" id="{$customers['.id']}" title="EDIT THIS CUSTOMER" style="color:#fff;font-size:17px;width:30px;height:30px;" class="btn btn-success btn-sm fa fa-pencil-square-o" aria-hidden="true"></a>
														{if $customers['permissions'] neq 'owner'}
															<a href="{$_url}mikrotik/usm-customers-remove&id={$customers['.id']}&name={$customers['login']}" id="{$customers['.id']}" title="REMOVE THIS CUSTOMER" onClick="return confirm('{$_L['API_Remove_Alert']}')" style="color:#fff;font-size:17px;width:30px;height:30px;" class="btn btn-warning btn-sm cdelete fa fa-trash" aria-hidden="true"></a>
														{/if}	
														</td>
													</tr>
												{/foreach}
				...
				<div  class="modal-dialog" role="document">
					<div class="modal-content">
						...
						<div class="modal-body">
							<form id="modal-form" class="form-horizontal" method="post" role="form" action="{$_url}mikrotik/usm-customers-add" >
