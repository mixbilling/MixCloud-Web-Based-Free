			...
			<script>
				function remove() {
					if(confirm('{$_L["API_Remove_Alert"]}')){
						document.getElementById("usm_profile").setAttribute("action", "{$_url}mikrotik/remove-selected-profiles");
											...
											<table  class="display table table-bordered table-bordered" id="asc-sort-col-1">
												...
												{foreach $profiles as $profile}
													...
													{/if}	
														...
														<td>{$profile['price']}</td>
														<td>
															<a href="{$_url}mikrotik/usm-profiles-edit&id={$profile['.id']}" id="{$profile['.id']}" title="EDIT THIS PROFILE" style="color:#fff;font-size:17px;width:30px;height:30px;" class="btn btn-success btn-sm fa fa-pencil-square-o" aria-hidden="true"></a>													
															<a href="{$_url}mikrotik/usm-profiles-remove&id={$profile['.id']}&name={$profile['name']}" id="{$profile['.id']}" title="REMOVE THIS PROFILE" onClick="return confirm('{$_L['API_Remove_Alert']}')" style="color:#fff;font-size:17px;width:30px;height:30px;" class="btn btn-warning btn-sm cdelete fa fa-trash" aria-hidden="true"></a>
														</td>
													</tr>
												{/foreach}
												</tbody>
